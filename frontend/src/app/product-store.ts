...
import { SeoManager } from '@shared/service/seo-manager';
  ...
  products: [
    {
      ...
      description:
        'High-quality over-ear headphones with active noise cancelation and 30-hour battery life.',
      price: 199.99,
...

    {
      ...
      description:
        'Track your workouts, heart rate, sleep, and daily activity with a sleek smart watch.',
      price: 149.99,
...

    {
      ...
      description:
        'Capture stunning videos and photos in extreme conditions with this rugged camera.',
      price: 229.99,
...
export const ProductStore = signalStore(
  ...
  withComputed(({ products, category, wishlist, cartItems }) => ({
    categories: computed(() => {
      ...
      return [{ name: 'all', link: ['/products', 'all'] }].concat(
        [...new Set(onlyCategories)].map((cat) => ({
          name: cat,
          link: ['/products', cat],
        })),
      );
  ...
  withMethods((store, toaster = inject(Toaster), seoManager = inject(SeoManager)) => ({
    ...
    addReview: ({ title, productId, rating, comment }: AddReviewParam) => {
      ...
      const updatedProducts = produce(store.products(), (draft) => {
        ...
        selectedProduct.reviews.push({
          ...
          date: formattedToday,
          avatar:
            'https://randomuser.me/api/portraits/men/' + Math.floor(Math.random() * 11) + '.jpg',
        });
      });
      patchState(store, {
    ...
    placeOrder: async () => {
      ...
      const order: Order = {
        ...
        total: store.cartItems().reduce((acc, item) => acc + item.product.price * item.qty, 0),
    ...
    moveItemToWishlist: (cartItem: CartItem) => {
      const updatedCart = store.cartItems().filter((ci) => ci.product.id != cartItem.product.id);
