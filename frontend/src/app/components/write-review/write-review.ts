      ...
      <form [formGroup]="reviewForm" (ngSubmit)="addReview()">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          ...
          <mat-form-field class="col-span-2">
            ...
            <textarea
              placeholder="Tell others about your experience with this product" formControlName="comment"
              matInput type="text" rows="4"
              required></textarea>
          </mat-form-field>
        </div>
        <div class="flex gap-4">
          <button matButton="filled" type="submit"> Submit Review</button>
          <button matButton="outlined" type="button" (click)="canceled.emit()">Cancel</button>
        </div>
      </form>
    </div>
  `,
